...
public class ClayRenderer {
	...
	public static void draw(LayoutResults results, GuiGraphics graphics, boolean textShadow) {
	...
	private static void drawRoundedRect(GuiGraphics graphics, int x, int y, int x2, int y2, CornerRadius cr, Color color) {
	...
	private static void fillRoundedScanlines(GuiGraphics graphics, int x, int y, int w, int h, CornerRadius cr, int col) {
	...
	private static void renderText(GuiGraphics graphics, Font font, RenderCommand cmd, BoundingBox box, boolean textShadow) {
		...
		int y = Math.round(box.y);
		if (textScale == 1.0f) {
			graphics.drawString(font, lineText, x, y, colorInt, textShadow);
			return;
		}
		graphics.pose().pushMatrix();
		graphics.pose().translate(x, y);
		graphics.pose().scale(textScale, textScale);
		graphics.drawString(font, lineText, 0, 0, colorInt, textShadow);
		graphics.pose().popMatrix();
	}
